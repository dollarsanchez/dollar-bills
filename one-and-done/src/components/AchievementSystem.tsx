...
function AchievementNotification({ achievement, isVisible, onClose }: AchievementNotificationProps) {
  useEffect(() => {
    if (isVisible) {
  ...
  return (
    <div className="fixed top-4 right-4 z-50 animate-fade-in-scale">
      <div className={`glass rounded-xl p-4 border-2 bg-gradient-to-r ${rarityColors[achievement.rarity]} border-white/30 shadow-2xl`}>
...
export default function AchievementSystem({ stats }: AchievementSystemProps) {
  ...
  useEffect(() => {
    const savedAchievements = JSON.parse(localStorage.getItem('one-and-done-achievements') || '[]');
  ...
  useEffect(() => {
    achievements.forEach(achievement => {
      if (!unlockedAchievements.includes(achievement.id) && achievement.condition(stats)) {
        ...
        localStorage.setItem('one-and-done-achievements', JSON.stringify(newUnlocked));
